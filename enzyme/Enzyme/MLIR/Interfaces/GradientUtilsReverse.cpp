...
// We assume that caches will only be written to at one location. The returned cache is (might be) "pop only"
...
Value MGradientUtilsReverse::popCache(Value cache, OpBuilder &builder) {
  return builder.create<enzyme::PopOp>(
      cache.getLoc(), cast<enzyme::CacheType>(cache.getType()).getInnerType(),
      cache);
}
